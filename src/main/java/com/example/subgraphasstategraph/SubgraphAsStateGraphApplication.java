package com.example.subgraphasstategraph;
...
public class SubgraphAsStateGraphApplication {

    private static final Logger log = LoggerFactory.getLogger(SubgraphAsStateGraphApplication.class);

    public static void main(String[] args) throws GraphStateException {
        StateGraph<State> workflowChild = createChildWorkflow();
        StateGraph<State> workflow = createWorkflow(workflowChild);
        CompiledGraph<State> compiledWorkflow = workflow.compile();

        log.info("Running parent workflow with child StateGraph node");
    ...
    private static StateGraph<State> createWorkflow(StateGraph<State> workflowChild) throws GraphStateException {
        return new StateGraph<>(State.SCHEMA, State::new)
