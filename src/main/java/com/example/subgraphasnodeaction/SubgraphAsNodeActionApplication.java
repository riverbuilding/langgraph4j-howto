package com.example.subgraphasnodeaction;
...
public class SubgraphAsNodeActionApplication {

    private static final Logger log = LoggerFactory.getLogger(SubgraphAsNodeActionApplication.class);
    ...
    public static void main(String[] args) throws GraphStateException {
        ...
        log.info("Running parent workflow with child graph invoked from a node action");
        AsyncGenerator<NodeOutput<State>> steps = compiledWorkflow.stream(Map.of());
        for (NodeOutput<State> step : steps) {
            log.info("{}", step);
        }

        //logGraph("sub graph", workflow.getGraph(GraphRepresentation.Type.PLANTUML, "sub graph", false));
    }
    ...
    private static StateGraph<State> createWorkflow(CompiledGraph<State> workflowChild) throws GraphStateException {
        return new StateGraph<>(State.SCHEMA, State::new)
                ...
                .addNode("subgraph", invokeSubgraph(workflowChild))
    ...
    private static AsyncNodeAction<State> invokeSubgraph(CompiledGraph<State> workflowChild) {
        return state -> {
            Map<String, Object> input = Map.of("subgraphKey", state.lastMessage().orElseThrow());
            for (NodeOutput<State> step : workflowChild.stream(input)) {
                log.info("subgraph: {}", step);
            }
